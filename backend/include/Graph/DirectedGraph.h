...
public:
    using Graph<T>::Graph;
    DirectedGraph(const string fileContent);
    DirectedGraph(const DirectedGraph<T>& other) : Graph<T>(other) {};
    ...
    std::map<T, std::vector<FlowEdge<T>>> convertGraph();
    std::string maxFlow(T s, T t) override;
    std::string toJSON() override;
};

#include "../../src/Graph/DirectedGraph.tpp"
#endif